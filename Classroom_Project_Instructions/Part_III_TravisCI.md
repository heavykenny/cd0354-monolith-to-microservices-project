...
4. Add a `.travis.yml` configuration file to the project directory locally. 
    ...
    # Build
      - docker build -t udagram-api-user-feed ./udagram-api-user-feed
    # Do similar for other three images
    ```

    ```bash
    # Tagging
      - docker tag udagram-api-user-feed sudkul/udagram-api-user-feed:v1
    # Do similar for other three images```
    ```bash
    # Push
    # Assuming DOCKER_PASSWORD and DOCKER_USERNAME are set in the Travis repository settings
      - echo "$DOCKER_PASSWORD" | docker login -u "$DOCKER_USERNAME" --password-stdin
      - docker push sudkul/udagram-api-user-feed:v1
    # Do similar for other three images
    ```
